...
def main(protocol_txt: str, audio_dir: str,checkpoint_dir:str, out_txt: str, utt_col: int = 1, ext: str = ".flac", layer: int = 7,
         device_str: str = "cuda"):
    device = torch.device(device_str if (device_str.startswith("cuda") and torch.cuda.is_available()) else "cpu")
    print("Using device:", device, flush=True)
    model = MaskedProsodyModelVAD.from_pretrained(checkpoint_dir)
    model = model.to("cpu")
    model.eval()
    try:
